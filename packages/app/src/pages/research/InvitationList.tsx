...
export function InvitationList() {
    ...
    // Forced casting as 'any' at the query level resolves the 'never' and 'implicitly any' errors
    ...
    const createMutation = useMutation({
        ...
        onSuccess: (data: any) => {
            ...
            setLabel('');
            if (data?.token) void copyLink(data.token);
        },
    });

    // Helper arrays for cleaner mapping
    const scenarioList = scenarios || [];
    const presetList = presets || [];
    const invitationList = invitations || [];

    const defaultPreset = presetList.find((p: any) => p.isDefault)?.name || presetList[0]?.name || '';
            ...
            <div className="mb-8 rounded-lg border border-gray-200 bg-white p-6 shadow">
                ...
                <div className="flex flex-wrap items-end gap-4">
                    <div className="min-w-[200px]">
                        ...
                        >
                            <option value="">{allowCustomScenario ? 'User describes...' : 'Select scenario...'}</option>
                    ...
                    <div className="min-w-[180px]">
                        <label className="mb-1 block text-sm font-medium text-gray-700">Quota</label>
                        <select
                            value={effectivePreset}
                            onChange={(e) => setPresetName(e.target.value)}
                            className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm"
                        >
                            {presetList.map((p: any) => (
                ...
                {loadingInvitations ? (
                    ...
                    <div className="divide-y divide-gray-200">
                        {invitationList.map((inv: any) => (
                            <Link key={inv.id} to={`/research/invitations/${inv.id}`} className="flex items-center gap-4 px-6 py-4 hover:bg-gray-50">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="text-sm text-gray-500">
                                        {inv.scenario?.name || (inv.allowCustomScenario ? 'User-defined' : 'No Scenario')}
                                    </div>
                                </div>
                                <button
                                    type="button"

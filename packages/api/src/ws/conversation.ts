...
function normalizeTone(value: unknown): LappTone {
  ...
  if (tone === 'constructive' || tone === 'warm' || tone === 'tense') return tone;
  return 'neutral';
}
...
export class ConversationManager {
  ...
  private countParticipantTurns(): number {
    return this.session.messages.filter((m) => m.role === 'user' && m.messageType !== 'aside')
      .length;
  }
  ...
  async handleResume(afterMessageId?: string | number): Promise<void> {
    ...
    const messages = afterIndex >= 0 ? allMessages.slice(afterIndex + 1) : allMessages;
        ...
        try {
          ...
          for await (const chunk of streamCompletion(currentModel, {
            ...
            if (chunk.type === 'delta' && chunk.content) {
              ...
              if (shouldEmergencyFallback(role, currentModel, usedEmergencyFallback, attempt)) {
                this.logger.warn(
                  {
                    sessionId: this.session.id,
                    role,
  ...
  private async runLappScorer(
    ...
    try {
      ...
      const timeout = setTimeout(() => controller.abort(), LAPP_SCORE_TIMEOUT_MS);
      let content = '';
      ...
      if (!parsed) {
        throw new Error(`LAPP scorer returned non-JSON output: ${content.slice(0, 120)}`);
      }
      ...
      await this.persistLappScore(userMessageId, turnNumber, scores, tone, 'ai', model);

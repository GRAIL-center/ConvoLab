...
import { adminProcedure, protectedProcedure, publicProcedure, router } from '../procedures.js';
...
export const invitationRouter = router({
   ...
   */
  claim: publicProcedure.input(z.object({ token: z.string() })).mutation(async ({ ctx, input }) => {
    const invitation = await ctx.prisma.invitation.findUnique({
      where: { token: input.token },
      include: {
        scenario: {
    ...
    if (invitation.linkedUserId && invitation.linkedUserId !== ctx.userId) {
      throw new TRPCError({
        code: 'BAD_REQUEST',
        message: 'This invitation has already been claimed by another user',
      });
    }

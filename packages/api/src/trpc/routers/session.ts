	...
	startNew: protectedProcedure
		.input(
			z
				...
				.refine(
					(data) =>
						data.scenarioId || (data.customDescription && data.elaborated),
					{
						message:
							"Either scenarioId or customDescription+elaborated must be provided",
					},
				),
		)
		.mutation(async ({ ctx, input }) => {
			...
			if (!customDescription || !elaborated) {
				throw new TRPCError({
					code: "BAD_REQUEST",
					message:
						"Custom scenario requires description and elaborated prompts",
				});
			}
		...
		return Promise.all(
			(sessions as SessionSummarySource[]).map(async (s) => {
				const [scenario, fallbackMessageCount] = await Promise.all([
					...
					numericCount(s.totalMessages) === null &&
					!s.messages &&
					!s._count?.messages

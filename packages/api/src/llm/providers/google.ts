...
function getClient(): GoogleGenAI {
	if (!genAI) {
		...
		if (project) {
			...
			if (!apiKey) {
				throw new Error(
					"Either GOOGLE_CLOUD_PROJECT (Vertex AI) or GOOGLE_AI_API_KEY (AI Studio) must be set",
				);
			}
			...
			for await (const chunk of response) {
				...
				if (!firstResponseDone) {
					...
					if (finishReason && finishReason !== "STOP") {
						console.log("[Google Provider] finishReason:", finishReason);
					}
